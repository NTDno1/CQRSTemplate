        ...
        where TEntity : EntityBase<TKey>
{
        ...
        params Expression<Func<TEntity, object>>[] includeProperties)
    {
        ...
        // Tạo câu truy vấn mặc định
        IQueryable<TEntity> items = _context.Set<TEntity>()
                                            .AsNoTracking();

        // Kiểm tra và áp dụng điều kiện "IsDeleted" nếu thuộc tính này tồn tại
        if (isDeletedProperty != null)
        {
            items = items.Where(e => !EF.Property<bool>(e, "IsDeleted"));
        }

        // Thực hiện include nếu có
        foreach (var includeProperty in includeProperties)
        {
            items = items.Include(includeProperty);
        }

        // Thực hiện điều kiện lọc theo predicate nếu có
