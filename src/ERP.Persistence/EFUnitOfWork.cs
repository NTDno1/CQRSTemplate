...
public class EFUnitOfWork : IUnitOfWork
{
    ...
    private void ConvertDomainEventsToOutboxMessages()
    {
        var outboxMessages = _dbContext.ChangeTracker
            .Entries<ERP.Domain.Abstractions.Aggregates.AggregateRoot>()
            .Select(x => x.Entity)
    ...
    private void UpdateAuditableEntities()
    {
        ...
        foreach (EntityEntry<IAuditTable> entityEntry in entries)
        {
            ...
            {
                entityEntry.Property(a => a.NgayTao).CurrentValue = DateTime.UtcNow;
            }

            if (entityEntry.State == EntityState.Modified)
            {
                entityEntry.Property(a => a.NgaySua).CurrentValue = DateTime.UtcNow;
            }
        }
    }
}
